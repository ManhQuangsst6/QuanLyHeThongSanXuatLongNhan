...
import { FormOutlined, CloseOutlined, ExclamationCircleFilled,CheckOutlined } from '@ant-design/icons';
import { } from '../../API/Ingredient/ingredient';
import { GetListByUser,ComfirmByEmployee } from '../../API/Attendance/AttendanceAPI';
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                ...
                <CheckOutlined style={{ stroke:'green',color:'green',strokeWidth:200} }/>: 
                <CloseOutlined style={{ stroke:'red',color:'red',strokeWidth:200} }/>
            ),
            align: "center"
        },
        {
            ...
            align: "center",
            render: (_, record) => (
                record.ComfirmAmount < 1  && <Space  align="baseline" style={{display:'flex',justifyContent:'center'}} >
                    <a style={{fontSize:20,stroke:'red',color:'red',strokeWidth:200}} onClick={() => ComfirmData(record)} ><CloseOutlined color='red' /></a>
                </Space>
            ),
  ...
  setInputValues(updatedInputValues);
    ...
    const ComfirmData=(data)=>{
        confirm({
            ...
            cancelText: 'Quay lại',
            onOk() {
                ComfirmByEmployee(data.key).then(res => {
    ...
    const fetchRecords = (pageNum, pageSize, nameSearch,dateSearch) => {
        setLoading(true);
        GetListByUser({ pageNum, pageSize, nameSearch,dateSearch })
            .then((res) => {
        ...
        <div style={{ padding: 10 }}>
            ...
            <div style={{ marginTop: "16px", }}>
                <div style={{ display: 'flex', justifyContent: 'space-between',flexWrap:'wrap' }}>
                    <h3 style={{margin:0,lineHeight:'55px'}}><FormOutlined style={{ strokeWidth: "30",color:'blue',stroke:'blue',fontSize:20,fontWeight:800, marginRight:8}}/>Bảng công </h3>
                <DatePicker placeholder='Từ' style={{height:34,margin:10}} onChange={onChangeDateSearch} />
             </div>
