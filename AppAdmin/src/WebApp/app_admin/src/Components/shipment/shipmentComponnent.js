...
const ShipmentComponent = () => {
  ...
  const fetchRecords = (pageNum, pageSize, nameSearch) => {
    setLoading(true);
    GetListShipmentByPage({ pageNum, pageSize, nameSearch }).then((res) => {
  ...
  const showModal = (state, dataEdit) => {
    ...
    if (state == "ADD") {
      ...
      ClearForm();
      var code = ConvertDateCode(new Date());
      SetDataPush({
        ...dataPush,
        shipmentCode: code,
      });
    ...
    if (dataEdit) {
      ...
      GetByID(dataEdit.key).then((res) => {
        ...
        SetDataRangDate([
          dayjs(res.data.value.dateTo, dateFormat),
          dayjs(res.data.value.dateFrom, dateFormat),
        ]);
        console.log(dataPush);
                ...
                <Row>
                  <Col span={12}>
                    <Form.Item
                      name="shipmentCode"
                      label="Mã lô hàng:"
                      rules={rules.shipmentCode}
                    >
                      <Input
                        value={dataPush.shipmentCode}
